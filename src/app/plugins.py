...
class Plugins:
    ...
    def load_plugin_dir(self, dir: Optional[Path] = None):
        ...
        if dir.is_file():
            try:
                ...
                self.plugins = self.plugins.union(map(lambda c: c(), inspect.getmembers(plugin, nfsplugin_predicate)))
    ...
    def load_prefixed_plugins(self, prefix: Optional[str] = None):
        ...
        self.plugins = self.plugins.union(map(lambda c: c(), inspect.getmembers(new_plugins, nfsplugin_predicate)))
    
    def add_plugins(self, plugins: Iterable[NFPlugin]):
        self.plugins = self.plugins.union(plugins)
